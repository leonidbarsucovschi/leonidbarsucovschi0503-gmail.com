...
<table width="100%"  border="0" cellpadding="0" cellspacing="0">
  <% String roleName = ((ro.uti.apmc.sys.model.AdmRoleVO)((ro.uti.apmc.sys.command.AdmRoleForm)request.getAttribute("form_AdmRoleForm")).getValues()).getRole_name(); %>
  <%-- tr>
  ...
  <tr>
  	<td colspan="4">
		<table width="100%" border="0" cellpadding="2" cellspacing="0"><tr
		  ...
		  <bean:message bundle="admuser" key="adm_user.roles.role.title" arg0="<%=roleName%>"/>
		</td>
  ...
  <logic:messagesPresent message="true">
    <html:messages id="meslogin" message="true" bundle="admuser">
      <tr>
        <td valign="top" colspan="4" class="errormsg" align="center">
  ...
  <tr>
	...
	<script type="text/javascript">
		...
		function addRoleNode(roleId) {
			var urlStr = "<%=request.getContextPath()%>/sys/admrole.do;jsessionid=<%=request.getSession().getId()%>?method=addRole&roleId="+roleId+"&type=<%=type%>";
			window.location = urlStr;
		}

		function removeRoleNode(roleId) {
			var urlStr = "<%=request.getContextPath()%>/sys/admrole.do;jsessionid=<%=request.getSession().getId()%>?method=removeRole&roleId="+roleId+"&type=<%=type%>";
			window.location = urlStr;
		}

		function saveUser() {
			var urlStr = "<%=request.getContextPath()%>/sys/admrole.do;jsessionid=<%=request.getSession().getId()%>?method=";
			urlStr += "save";
			window.location = urlStr;
		}
  ...
  <tr align="center">
    <td colspan="4">
<%--    	<button class="btn-generic" onclick="saveUser()"><bean:message bundle="admuser" key="adm_user.button.save"/></button>--%>
     <button class="btn-generic" onclick="javascript: document.getElementById('frm_save').submit();"><bean:message bundle="admuser" key="adm_user.button.save"/></button>
    </td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<html:form  action="/admrole.do"  styleId="frm_save">
