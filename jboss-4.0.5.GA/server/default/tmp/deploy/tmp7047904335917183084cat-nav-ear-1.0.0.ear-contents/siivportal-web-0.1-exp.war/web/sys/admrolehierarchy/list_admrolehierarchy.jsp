  ...
  <table width="100%" cellspacing="0" cellpadding="0" border="0">
    <tr class="bottom">
      ...
      <td width="71%" class="page-title-header" align="left">
        ...
        <b><bean:message bundle="admrolehierarchy" key="adm_role_hierarchy.title.list"/>
        </b><!-- END Sectiune de Titlu pagina. -->
      </td>
      <td>
        <html:form action="admrolehierarchy.do" styleId="addForm">
      ...
      <td valign="middle" nowrap="true" align="right">
        <a href="javascript:document.getElementById('addForm').submit()">
          <img border="0" src="images/add.gif"  title="<bean:message bundle="admrolehierarchy" key="adm_role_hierarchy.button.add"/>" />
        </a>
...
<%
        try { 
%>
          <jsp:include page="search_admrolehierarchy.jsp" flush="true"/>
<%
        } catch (Exception e) {
          e.printStackTrace();
        }
...
<%
  try { 
%>
    <jsp:include page="list_lista_admrolehierarchy.jsp" flush="true"/>
<%
  } catch (Exception e) {
    e.printStackTrace();
  }
...
<form name="actionForm" method="post" action="sys/admrolehierarchy.do">
  <input type="hidden" name="method" />
  <input type="hidden" name="page" />
  <input type="hidden" name="prefix" value="/" />
  <input type="hidden" name="id_role_hierarchy" />
</form>
<script>
  function showPrint(name, id) {
    link = "<html:rewrite page="/admrolehierarchy.do"/>";
