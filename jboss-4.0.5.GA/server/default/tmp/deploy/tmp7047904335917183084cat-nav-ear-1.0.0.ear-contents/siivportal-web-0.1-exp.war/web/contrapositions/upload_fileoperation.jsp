...
<script>
	...
	function MultiSelector(list_target, max)
	{
		...
		this.checkRadios = function(element)
		{
			...
			if (document.form_FileOperationForm.selFile != null)
			{
				for (var i=0; i < document.form_FileOperationForm.selFile.length; i++)
				{
					if (document.form_FileOperationForm.selFile[i].checked)
						found_it = true;
				}
				if (!found_it && document.form_FileOperationForm.selFile.length > 1)
					alert("Va rugam sa selectati fisierul principal.");
				else
					found_it = true;
			}
			else
				alert("Va rugam sa adaugati un fisier extern (si in cazul Ucraina - lunar, subfisierul sau) pentru incarcare.");
		...
		this.alreadyInList = function(value)
		{
			...
			if (document.form_FileOperationForm.selFile != null)
			{
				for (var i=0; i < document.form_FileOperationForm.selFile.length; i++)
				{
					if (document.form_FileOperationForm.selFile[i].value == value)
						found_it = true;
				}
				if (document.form_FileOperationForm.selFile.value == value)
					found_it = true;
			}
			return found_it;
		}
...
<div class="table">
	<table width="100%" border="0" cellpadding="0" cellspacing="1">
		<tr>
			<td width="100%" valign="top" align="left" class="page-subtitle">
				  <bean:message bundle="fileoperation" key="fileoperation.admin.title.new"/>
			</td>
			<td class="bottom" align="right">
				<img border="0" style="cursor:pointer;" onClick="closeWindow();"
					 title='<bean:message bundle="fileoperation" key="fileoperation.label.closeWindow"/>' src="images/close_off.gif"
		...
		<logic:messagesPresent>
			<tr>
				<td valign="top" colspan="2" align="center" class="msg_error">
					<html:messages id="error" bundle="fileoperation">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="report">
		...
		<html:form action="/fileoperation.do" enctype="multipart/form-data" onsubmit="return multi_selector.checkRadios()">
			<!-- Cimpul 'nume fisier' field. -->
			<tr>
				<td valign="top" class="label" align="right" width="50%">
					<bean:message bundle="fileoperation" key="fileoperation.prompt.uploadFile"/>:
				</td>
				<td align="left">
			...
			<tr>
				<td colspan="2" class="button-row" align="center">
					<table border="0" cellpadding="0" cellspacing="1">
						<tbody id="files_list">
						<tr>
							<th>&nbsp<bean:message bundle="fileoperation" key="fileoperation.prompt.mainFile"/>&nbsp</th>
							<th width="250"><bean:message bundle="fileoperation" key="fileoperation.prompt.uploadFile"/></th>
							<th width="50">&nbsp</th>
						</tr>
						</tbody>
					</table>
			...
			<tr>
				<td colspan="2" class="button-row" align="center">
					<html:submit styleClass="btn-generic">
						<bean:message bundle="fileoperation" key="fileoperation.button.upload"/>
					</html:submit>
