...
function choose(choiceLabel, choiceId) {
...
function chooseTag(choiceLabel, choiceId,choiceTag) {
	...
	if (isEdit == 0) {
		if ((choiceLabel.length == 0) || (choiceId == -1)) {
            ...
            if (choiceTag.length != 0){
                window.opener.document.getElementById('<bean:write name="choiceLabel"/>').value = choiceTag;
            }
            else{
                window.opener.document.getElementById('<bean:write name="choiceLabel"/>').value = choiceLabel;
            }
<%--
            Aceste linii au fost modificate pentru a functiona pe Netscape
...
--%>
            ...
            window.opener.document.getElementById('<bean:write name="choiceId"/>').value = choiceId;
...
b.add(<bean:write name="node" property="id"/>,
        <bean:write name="node" property="parentId"/>,
        '<bean:write name="node" property="label" filter="false"/>'
        ,'<bean:write name="node" property="action"/>',
        '<bean:write name="node" property="label" filter="false"/>', '', '');
</logic:iterate>
document.write(b);
</script>
