...
<%
	...
	for (int j = 0; j < jrParameters.length; j++)
	{
				...
				&& (visible == null || !visible.equalsIgnoreCase("false")))
		{
			...
			{
				%>	<input type="checkbox" name="p[<%=i%>].enabled" id="<%= "parametersEnabled" + i %>" value="true" onclick="javascript:updateParameter(<%=i%>)"><%
			}
...
<%if (tooltip != null){%><bean:message bundle="report" key="report.parameters.label.tooltip" arg0="<%=tooltip%>"/><%}%>"/>
				...
				<%
					...
					{
				%>
					<input type="text" name="p[<%=i%>].value2" value="" id="<%= "parametersValue2_" + i%>" class="field_input">
...
<%if (tooltip != null){%><bean:message bundle="report" key="report.parameters.label.tooltip" arg0="<%=tooltip%>"/><%}%>"/>
				...
				<%
					...
					{
				%>
						<input type="text" name="p[<%=i%>].value2" id="<%= "parametersValue2_" + i%>" value="" class="field_input">
				<%
					}
			}
			if (type != null && (type.equals(String.valueOf(java.sql.Types.VARCHAR))))
			...
			if (type != null && (type.equals("-100")))
			{
				...
				<td align="left">
					...
					<select align="absmiddle" name="p[<%=i%>].value1" id="<%= "parametersValue1_" + i%>" class="selectboxTag" onchange="<%= "clearSelects(" + i + ")"%>">
						<option value="hs2" <%=(length == 2?"SELECTED":"")%>>
							Nomenclator 2 cifre
						</option>
						<option value="hs4" <%=(length == 4?"SELECTED":"")%>>
							Nomenclator 4 cifre
						</option>
						<option value="hs6" <%=(length == 6?"SELECTED":"")%>>
							Nomenclator 6 cifre
						</option>
						<option value="hs8" <%=(length == 8?"SELECTED":"")%>>
							Nomenclator 8 cifre
						</option>
					</select>
				</td>
						...
						%>
							...
							<td align="left" colspan="2">
								...
								<select align="absmiddle" name="order2" id="order2" class="selectboxTag">
									<option value="<%=SIIVConstants.SORT_ORDER_ASCENDING_STR%>">
										<bean:message bundle="report" key="report.parameters.operators.text.Asc"/>
									</option>
									<option value="<%=SIIVConstants.SORT_ORDER_DESCENDING_STR%>">
...
<tr>
    <td colspan="5" class="button-row" align="center">
        ...
        <html:hidden property="method" value="execReport"/>
    </td>
</tr>
